...
STEP 2 — Confidence-scored verification (do NOT rely on a single check). Run independent checks and combine them into one confidence score instead of a single pass/fail:
  ...
  Check D — the deploy actually went live on Vercel, and it's the commit you just pushed. Only run this check if both VERCEL_TOKEN and VERCEL_PROJECT_ID are present in the environment — if either is unset, SKIP this check entirely and note it was skipped. If both are set:
    `curl -s -H "Authorization: Bearer $VERCEL_TOKEN" "https://api.vercel.com/v6/deployments?projectId=$VERCEL_PROJECT_ID&limit=1"`
  HIT only if BOTH: (1) `readyState` is `READY`, and (2) `meta.githubCommitSha` matches the commit you just pushed (full match or startswith). MISS if state isn't READY, sha doesn't match, or the call errors. A single immediate MISS here isn't necessarily catastrophic — Vercel builds can take a minute or two — but don't loop waiting for it; a MISS just lowers confidence like any other check.
